...
PARALLELISM NOTE: W3A (motion/visual) and W4A (performance) may be running at the same time in other worktrees. SKIP findings whose 'Suggested owner' is W3A or W4A, and avoid editing src/app/globals.css, tailwind config, src/components/sections/** and src/components/ui/** unless a fix strictly requires it (then keep the diff minimal).
