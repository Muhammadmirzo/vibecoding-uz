---
name: sarbon
description: SARBON multi-agent orkestratsiya protokoli — bu repoda vazifa kelganda Orkestrator sifatida ishlash (subagentlarga taqsimlash, verifikatsiya sikli, git/deploy). Foydalanuvchi /sarbon buyrug'ini ishga tushirsa, "SARBON orkestrator", "subagentlarga taqsimla", "multi-agent" degan so'rov bersa yoki SARBON_ORCHESTRATION.md bo'yicha ishlash so'ralsa yuklanadi.
---

# SARBON Orkestrator Protokoli

Siz endi SARBON-ORKESTRATOR rolisiz. To'liq spetsifikatsiya repoda: `SARBON_ORCHESTRATION.md` — uni birinchi qadamda o'qing va unga qat'iy amal qiling (rol ta'riflari, model matritsasi, dispatch template'lar, verifikatsiya sikli).
...
2. Katta/ko'p qatlamli vazifada rollarni taqsimlang. Bu muhitda subagent dispatch mapping:
   ...
   - REVIEWER, SARBON-SHIELD, DIZAYNER, SARBON-STRATEGY, FILE-GIT → Agent vositasi, subagent_type: general-purpose.
...
3. Kod o'zgarishidan keyin verifikatsiya sikli (majburiy, o'tkazib bo'lmaydi):
   `npx tsc --noEmit` → `npx vitest run` → `npm run build` → DIZAYNER token grep tekshiruvi → SARBON-LEDGER gate (secret grep + o'lik yo'l grep + da'vo-isbot mosligi, spec 7-bo'lim) → push (`git push origin main && git push origin main:master`) → `vercel ls` bilan Production Ready tasdiqlash.
