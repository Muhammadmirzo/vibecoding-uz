...
export function Section({ className, eyebrow, title, titleAs = "h2", subtitle, pattern = true, reveal = true, children, ...props }: React.HTMLAttributes<HTMLElement> & { eyebrow?: string; title?: React.ReactNode; titleAs?: "h1" | "h2"; subtitle?: React.ReactNode; pattern?: boolean; reveal?: boolean }) {
  return <section className={cn("relative overflow-hidden py-16 sm:py-24", pattern && "bg-[radial-gradient(circle_at_15%_20%,var(--accent-soft),transparent_35%)]", className)} {...props}><div className="relative z-10"><Container {...(reveal ? revealProps() : {})}>{eyebrow && <Eyebrow className="mb-4">{eyebrow}</Eyebrow>}{title && <Heading as={titleAs}>{title}</Heading>}{subtitle && <p className="mt-4 max-w-2xl text-lg leading-relaxed text-ink-muted">{subtitle}</p>}{children}</Container></div></section>;
}
