...
export function CourseCheckoutButton({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className={`btn-press inline-flex w-full items-center justify-center gap-2 rounded-lg bg-gold font-semibold text-ink shadow-sm transition-colors hover:bg-gold-hover ${compact ? "min-h-11 px-4 text-sm" : "min-h-12 px-5 text-base"}`}
    >
