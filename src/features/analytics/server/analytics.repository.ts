...
export class DrizzleAnalyticsRepository implements AnalyticsRepository {
  ...
  async students(range: AnalyticsRange) {
    const [summary, courseRows, attendanceRows] = await Promise.all([
      ...
      rows(sql`SELECT c.id AS "courseId", c.title AS "courseTitle", count(DISTINCT e.user_id)::int AS started, count(lp.completed_at)::int AS completed, round(100.0 * count(lp.completed_at) / NULLIF(count(lp.user_id), 0), 1) AS "completionRate", (SELECT l2.title FROM lessons l2 JOIN course_sections s2 ON s2.id = l2."sectionId" LEFT JOIN lesson_progress lp2 ON lp2."lessonId" = l2.id GROUP BY l2.id ORDER BY count(lp2.completed_at) ASC NULLS FIRST LIMIT 1) AS "dropOffLesson" FROM courses c LEFT JOIN course_sections s ON s."courseId" = c.id LEFT JOIN lessons l ON l."sectionId" = s.id LEFT JOIN lesson_progress lp ON lp."lessonId" = l.id LEFT JOIN enrollments e ON e."cohortId" IN (SELECT id FROM cohorts WHERE "courseId" = c.id) GROUP BY c.id ORDER BY started DESC`),
      rows(sql`SELECT count(DISTINCT a.user_id) FILTER (WHERE a.type = 'lesson_start')::int AS attended, (SELECT count(*)::int FROM enrollments e2 JOIN cohorts c2 ON c2.id = e2."cohortId" WHERE e2.status = 'active') AS total FROM analytics_events a JOIN enrollments e ON e."userId" = a.user_id JOIN cohorts c ON c.id = e."cohortId" WHERE c."startsAt" <= a.occurred_at AND (c."endsAt" IS NULL OR c."endsAt" >= a.occurred_at)`),
    ]);
  ...
  async sales(range: AnalyticsRange) {
    const [daily, byCourse, refunds, referrers] = await Promise.all([
      ...
      rows(sql`SELECT c.id AS "courseId", c.title AS "courseTitle", coalesce(sum(p.amount_sum), 0)::int AS "revenueUzs", count(p.id)::int AS orders FROM courses c LEFT JOIN cohorts co ON co."courseId" = c.id LEFT JOIN enrollments e ON e."cohortId" = co.id LEFT JOIN payments p ON p."enrollmentId" = e.id AND p.status = 'paid' AND p."paidAt" >= ${range.from} AND p."paidAt" < ${range.to} GROUP BY c.id ORDER BY "revenueUzs" DESC`),
      rows(sql`SELECT count(*)::int AS count, coalesce(sum(amount_sum), 0)::int AS value FROM payments WHERE status = 'refunded' AND "paidAt" >= ${range.from} AND "paidAt" < ${range.to}`),
