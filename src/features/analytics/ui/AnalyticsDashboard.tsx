...
function read<T>(schema: z.ZodType<T>, body: unknown): T | undefined { const envelope = z.object({ data: z.unknown() }).safeParse(body); if (!envelope.success) return undefined; const parsed = schema.safeParse(envelope.data); return parsed.success ? parsed.data : undefined; }
