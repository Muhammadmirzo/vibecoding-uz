...
export function ServicesProcessAndFaq() {
  ...
  return <>
    ...
    <Section eyebrow="Ochiq savollar" title="Avval bilish kerak." subtitle="Javob topilmasa, Telegram orqali bepul savol bering."><Accordion type="single" collapsible className="max-w-3xl border-t border-border">{page.faq.map((item) => <AccordionItem key={item.question} value={item.question}><AccordionTrigger>{item.question}</AccordionTrigger><AccordionContent>{item.answer}</AccordionContent></AccordionItem>)}</Accordion></Section>
