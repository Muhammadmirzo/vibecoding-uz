        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-lg max-h-[90vh] overflow-y-auto bg-cream border border-border-strong rounded-2xl p-6 md:p-8 shadow-2xl animate-in zoom-in-95 duration-150 text-ink">
          <div className="flex items-start justify-between pb-4 border-b border-border">
            ...
            <Dialog.Close asChild>
              <button className="w-8 h-8 rounded-lg flex items-center justify-center text-ink-subtle hover:text-ink hover:bg-cream-warm transition-colors" aria-label="Yopish">
                <X className="w-5 h-5" />
              </button>
            </Dialog.Close>
          </div>
          {form.success ? (
            <div className="py-10 text-center space-y-4">
              ...
              <div className="pt-4">
                <button onClick={onClose} className="btn-primary h-11 px-8 rounded-lg text-xs font-semibold">Tushunarli</button>
              </div>
            </div>
          ) : (
