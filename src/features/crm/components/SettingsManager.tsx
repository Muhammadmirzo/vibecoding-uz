...
import { McpSettingsPanel } from "@/features/mcp/ui/McpSettingsPanel";
...
const tabs: Array<{ id: SettingsTab; label: string; icon: typeof Globe }> = [
  ...
  { id: "motion", label: "Animatsiyalar", icon: Sparkles }, { id: "security", label: "Xavfsizlik", icon: Lock }, { id: "mcp", label: "MCP ulanishlar", icon: Key },
];
...
export function SettingsManager() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
      ...
      {activeTab !== "security" && activeTab !== "motion" && activeTab !== "mcp" && <button onClick={() => void save()} disabled={saving} className="flex h-11 items-center justify-center gap-2 rounded-xl bg-gold px-5 text-sm font-semibold text-ink disabled:opacity-50">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}Sozlamalarni saqlash</button>}
    </div>
    ...
    {activeTab === "mcp" ? <McpSettingsPanel /> : loading ? <div className="flex justify-center py-20"><Loader2 className="h-8 w-8 animate-spin text-accent" /></div> : activeTab === "security" ? <SecurityTab settings={settings} setSettings={setSettings} /> : <form onSubmit={(event) => { event.preventDefault(); void save(); }} className="rounded-xl border border-border bg-bg-sunken p-4 sm:p-6">
