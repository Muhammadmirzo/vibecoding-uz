...
export function SecurityTab({ settings, setSettings }: Props) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/auth/change-password", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ phone: settings.credPhone, email: settings.credEmail, oldPassword: settings.oldPassword, newPassword: settings.newPassword, confirmPassword: settings.confirmPassword }) });
  ...
  return <form onSubmit={submit} className="space-y-6 rounded-xl border border-border bg-bg-sunken p-6">
    ...
    {message && <p role="status" className="rounded-lg border border-border bg-accent-soft p-3 text-sm text-accent">{message}</p>}
    ...
    <button type="submit" disabled={saving} className="ml-auto flex h-11 items-center gap-2 rounded-lg bg-accent px-5 text-sm font-semibold text-ink disabled:opacity-50"><Save className="h-4 w-4" />{saving ? "Saqlanmoqda…" : "Login va parolni yangilash"}</button>
  </form>;
}
