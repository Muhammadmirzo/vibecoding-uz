...
/** Visitor-safe subset: AI provider/model/persona/caps stay server-side. */
export function publicChatSettings(settings: ChatSettingsInput) {
  const { enabled, welcomeText, officeHours, replyTimeMinutes, offlineText, quickReplies } = settings;
  return { enabled, welcomeText, officeHours, replyTimeMinutes, offlineText, quickReplies };
}
