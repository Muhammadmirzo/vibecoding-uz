  ...
  return (
    <article className="lg:col-span-8 bg-bg-elevated border border-border-strong rounded-2xl p-6 md:p-10 shadow-sm">
      ...
      <div className="mt-10 pt-6 border-t border-border flex flex-wrap items-center gap-2">
        ...
        {post.tags.map((tag) => (
          <span key={tag} className="text-xs font-mono px-3 py-1 rounded-md bg-bg text-ink border border-border">#{tag}</span>
        ))}
      </div>
      <div className="mt-8 p-6 rounded-xl bg-bg border border-border flex flex-col sm:flex-row items-center justify-between gap-4">
