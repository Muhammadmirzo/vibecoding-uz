...
export function NotificationToggles(props: NotificationState) {
  ...
  return <div className="mt-6 space-y-6">
    ...
    <div className="flex flex-col items-end gap-3 border-t border-border pt-5"><Button onClick={handleSaveNotifications}><Save className="h-4 w-4" aria-hidden="true" />{notifSaved ? "Tanlovlar saqlandi" : "Tanlovlarni saqlash"}</Button><p className="text-sm text-ink-subtle">Bu tanlovlar faqat shu brauzerda saqlanadi; serverga yuborilmaydi.</p><p className="sr-only" aria-live="polite">{notifSaved ? "Tanlovlar saqlandi" : ""}</p></div>
  </div>;
}
